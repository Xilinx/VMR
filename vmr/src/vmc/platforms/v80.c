...
AsdmHeader_info_t xV80ASDMHeaderInfo[] = 
{
...
#else
    scStatus =  ucISL68221ReadTemperature0( i2c_main, SLAVE_ADDRESS_ISL68221_1, &sensor_glvr.sensor_readings.fVccHbmTemp );
#endif
    if( XST_FAILURE == scStatus )
    {
...
#else
    scStatus =  ucISL68221ReadTemperature1( i2c_main, SLAVE_ADDRESS_ISL68221_1, &sensor_glvr.sensor_readings.f1V2VccoDimmTemp );
#endif
    if( XST_FAILURE == scStatus )
    {
...
#else
    scStatus =  ucISL68221ReadTemperature2( i2c_main, SLAVE_ADDRESS_ISL68221_1, &sensor_glvr.sensor_readings.f1V2GtxaVttTemp );
#endif
    if( XST_FAILURE == scStatus )
    {
...
s32 slV80VMCFetchBoardInfo( u8 *board_snsr_data )
{
    ...
    return ( ( ulByteCount <= MAX_VMC_SC_UART_BUF_SIZE ) ? ( ulByteCount ) : ( -1 ) );
}
...
#else
    u8 scStatus     = XST_FAILURE;
    scStatus = INA3221_ReadVoltage( i2c_main, SLAVE_ADDRESS_INA3221_1, 0, &fVoltage );
    if( XST_SUCCESS != scStatus )
    {
...
#else
    u8 scStatus     = XST_FAILURE;
    scStatus = INA3221_ReadCurrent( i2c_main, SLAVE_ADDRESS_INA3221_1, 0, &fCurrent );
    if( XST_SUCCESS != scStatus )
    {
...
#else
    u8 scStatus     = XST_FAILURE;
    scStatus = INA3221_ReadVoltage( i2c_main, SLAVE_ADDRESS_INA3221_1, 1, &fVoltage );
    if( XST_SUCCESS != scStatus )
    {
...
#else
    u8 scStatus     = XST_FAILURE;
    scStatus = INA3221_ReadCurrent( i2c_main, SLAVE_ADDRESS_INA3221_1, 1, &fCurrent );
    if( XST_SUCCESS != scStatus )
    {
...
#else
    u8 scStatus     = XST_FAILURE;
    scStatus = INA3221_ReadVoltage( i2c_main, SLAVE_ADDRESS_INA3221_1, 2, &fVoltage );
    if( XST_SUCCESS != scStatus )
    {
...
#else
    u8 scStatus     = XST_FAILURE;
    scStatus = INA3221_ReadCurrent( i2c_main, SLAVE_ADDRESS_INA3221_1, 2, &fCurrent );
    if( XST_SUCCESS != scStatus )
    {
...
#else
    u8 scStatus     = XST_FAILURE;
    scStatus =  ucISL68221ReadVoltage0( i2c_main, SLAVE_ADDRESS_ISL68221_1, &fVoltage );
    if( XST_SUCCESS != scStatus )
    {
...
void vV80CurrentMonitorVccHBM( )
{
    ...
    scStatus =  ucISL68221ReadCurrent0( i2c_main, SLAVE_ADDRESS_ISL68221_1, &fCurrentInA );
    if( XST_SUCCESS != scStatus )
    {
...
#else
    u8 scStatus     = XST_SUCCESS;
    scStatus =  ucISL68221ReadVoltage1( i2c_main, SLAVE_ADDRESS_ISL68221_1, &fVoltage );
    if( XST_SUCCESS != scStatus )
    {
...
#else
    u8 scStatus         = XST_SUCCESS;
    scStatus =  ucISL68221ReadCurrent1( i2c_main, SLAVE_ADDRESS_ISL68221_1, &fCurrentInA );
    if( XST_SUCCESS != scStatus )
    {
...
#else
    u8 scStatus     = XST_SUCCESS;
    scStatus =  ucISL68221ReadVoltage2( i2c_main, SLAVE_ADDRESS_ISL68221_1, &fVoltage );
    if( XST_SUCCESS != scStatus )
    {
...
#else
    u8 scStatus         = XST_SUCCESS;
    scStatus =  ucISL68221ReadCurrent2( i2c_main, SLAVE_ADDRESS_ISL68221_1, &fCurrentInA );
    if( XST_SUCCESS != scStatus )
    {
...
void vV80PowerMonitor( )
{
    ...
    if( POWER_12VPEX_CRITICAL_THRESHOLD  <= ( fPower12vPex ) )
    {
    ...
    {
        if( ( 0 != ucCount12vpex ) && ( POWER_12VPEX_CRITICAL_THRESHOLD > ( fPower12vPex ) ) )
        {
            ucCount12vpex = 0;
        }
    }
    if( POWER_3V3PEX_CRITICAL_THRESHOLD <= ( fPower3v3pex ) )
    {
    ...
    {
        if( ( 0 != ucCount3v3pex ) && ( POWER_3V3PEX_CRITICAL_THRESHOLD > ( fPower3v3pex )  ) )
        {
            ucCount3v3pex = 0;
        }
...
s8 scV80AsdmTemperatureReadVccint( snsrRead_t *pxSnsrData )
{
    ...
    if ( usTemp >= V80_TEMP_VCCINT_CRITICAL_THRESHOLD )
    {
        ucs_clock_shutdown( );
    }
    return scStatus;
...
s8 scV80AsdmGetTemperatureNames( u8 ucIndex, char8* pucSnsrName, u8 *pucSensorId, sensorMonitorFunc *pxSensorHandler )
{
    ...
    struct sensorData xSnsrData[] =    {
        //{  "PCB\0",             scV80AsdmTemperatureReadBoard },
        //{  "VCCINT\0",          scV80AsdmTemperatureReadVccint },
