...
u8 ucCAT34TS02ReadTemperature( u8 ucI2cNum, u8 ucSlaveAddr, s16 *pssTemperatureValue )
{
	...
	*pssTemperatureValue = 38;
	...
	else
    {
		...
		usTempHexVal = usTempHexVal >> 7;

		const int negative = ( usTempHexVal & ( 1 << 8 ) ) != 0;
		usTempHexVal &= ( 0xFF );
		s16 nativeInt = 0;

		if( negative )
		{
  			nativeInt = usTempHexVal | ~( ( 1 << 8 ) - 1) ;
			*pssTemperatureValue = ( s16 )( nativeInt/2 );
		}
		else
		{
...
{
	u8 ucStatus = 0;
	ucStatus = i2c_send_rs_recv(ucI2cNum, ucSlaveAddr, (u8 *)pusAddressOffset, EEPROM_ADDRESS_SIZE, pucRegisterValue, xBufSize);
