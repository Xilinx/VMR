...
static void test_SE98A_ReadTemperature(void **state) {
	(void) state; /* unused */

	int ret = 0;
	s16 temperatureValue = 0;
	...
	for(i = 0; i < 10; i++)
	{
		...
		set_Buffer(&read_buff[0], 0, 1360, 2);
		...
		/*Test Function*/
		ret = SE98A_ReadTemperature(SE98A_I2CNUM_1, SLAVE_ADDRESS_SE98A_0, &temperatureValue);

		/*Assert results*/
		assert_true(ret == 0);
		assert_in_range(temperatureValue, SE98A_POSITIVE_MIN_TEMP, SE98A_POSITIVE_MAX_TEMP);
	}
}
...
static void test_Negative_SE98A_ReadTemperature(void **state) {
	...
	for(i = 0; i < 10; i++)
	{
		...
		set_Buffer(&read_buff[0], 6144, 8176, 2);
		...
		/*Assert results*/
		assert_true(ret == 0);
		assert_in_range((temperatureValue & 0xfff), (SE98A_NEGATIVE_MIN_TEMP & 0xfff), (SE98A_NEGATIVE_MAX_TEMP & 0xfff));
	}
}
...
int main(void) 
{
	...
	const struct CMUnitTest tests[] = {
			cmocka_unit_test(test_SE98A_ReadTemperature),
