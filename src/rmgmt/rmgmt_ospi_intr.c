...
/*
 * Author: David Zhang davidzha@xilinx.com
 *   This file is modifed from xospipsv_flash_intr_example.c
 */
...
static void OspiPsvHandler(void *CallBackRef, u32 StatusEvent);
...
int ospi_flush_intr(u8 *WriteBuffer, u32 len)
{
	...
	for (Count = 0; Count < len; Count += PAGE_SIZE ) {
		if (Count % (PAGE_COUNT / 10))
			continue;
		xil_printf("\rRead back offset %d", Count);
		fflush(stdout);
