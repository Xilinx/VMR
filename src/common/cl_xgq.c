...
/* Temporary use bottom 4k for XGQ polling mode */
#if defined(XGQ_DEBUG_IP)
//#define RPU_RING_BASE (XPAR_PSV_OCM_RAM_0_S_AXI_BASEADDR + 0x8000)
#define RPU_RING_BASE (0xFFFE0000)
#define RPU_SQ_BASE (RPU_RING_BASE + 0x6000)
#define RPU_CQ_BASE (RPU_RING_BASE + 0x7000)
#else
#define RPU_RING_BASE (0xFFFE0000)
...
static int submit_to_queue(u32 sq_addr)
{
	...
	MSG_LOG("msg 0x%x", xgq_mem_read32(0x0, (u32)cmd + sizeof(*cmd)));
	msg.pkt.head.cid = cmd->cid; //remember the cid, cmd will be freed
	msg.pkt.head.cid = 0xff; //remember the cid, cmd will be freed
	MSG_LOG("msg %d", msg.pkt.head.version);
	MSG_LOG("msg %d", msg.pkt.head.type);
	MSG_LOG("msg %d", msg.pkt.head.cid);
	MSG_LOG("msg %d", msg.pkt.head.rcode);
...
static int init_xgq()
{
	...
	MSG_LOG("->");
	MSG_LOG("write range 0x%x to 0x%x all zero", RPU_RING_BASE, RPU_RING_BASE + 0x7fff);
	{
		 u32 i;
		 for (i = RPU_RING_BASE; i < RPU_RING_BASE + 0x8000; i += 4) {
			 //xil_printf("%x\r", i);
