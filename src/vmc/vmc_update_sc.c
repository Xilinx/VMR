...
uint8_t bsl_send_data_pkt[BSL_MAX_DATA_SIZE] = {0x00};
uint8_t addr_flag = 0x00;
uint32_t addr = 0x00;
uint16_t total_length = 0x00;
uint32_t data_ptr = 0;
uint32_t Prev_length = 0x00;
uint16_t CRC_Val = 0x00;
uint16_t dataCRC = 0x00;
uint8_t msg[1] = {0x00};
bool all_pkt_sent = false;
uint8_t fpt_sc_version[3] = {0x00};

uint8_t sc_update_required = 0x00;

/* Variable to keep track whether SC update is going on or not */
uint8_t sc_update_flag = 0x00;

/* Variables will keep track of SC update progress */
uint32_t update_progress = 0;
uint32_t curr_prog = 0;
uint32_t max_data_slot = 0;


uint8_t receive_bufr[32] = {0xFF};
uint32_t receivedByteCount = 0x00;

uint8_t bslPasswd[BSL_UNLOCK_PASSWORD_REQ]	=	{ 0x80,0x39,0x00,0x21,0x58,0x41,0x50,0x3A,0x20,0x58,0x69,
						  	  	  	  	  	  	  0x6C,0x69,0x6E,0x78,0x20,0x41,0x6C,0x6C,0x20,0x50,0x72,0x6F,
												  0x67,0x72,0x61,0x6D,0x6D,0x61,0x62,0x6C,0x65,0x2C,0x20,0x58,0x42,0x42,
												  0x3A,0x20,0x58,0x69,0x6C,0x69,0x6E,0x78,0x20,0x42,0x72,0x61,0x6E,
												  0x64,0x65,0x64,0x20,0x42,0x6F,0x61,0x72,0x64,0x73,0x82,0xE5 };

extern SC_VMC_Data sc_version;
...
{
	uint8_t read_buffer[SC_HEADER_SIZE] = {0};
	uint8_t header[SC_HEADER_SIZE] = SC_HEADER_MSG;
	uint8_t retVal = 0x01;
	data_ptr = 0x00;
...
void VMC_Parse_Fpt_SC_Version(uint32_t addr_location, uint8_t *versionbuff)
{
	uint8_t state = PARSE_ADDRESS;
	uint8_t	Complete_flag = 0;
	bool version_flag = false;
	uint8_t version_ptr = 0x00;
	...
	while (Complete_flag == 0)
	{
		...
		case PARSE_DATA:
			...
			{
				if (msg[0] != 0x0A && msg[0] != 0x20)
				{
					msg[0] = Char2Hex(msg[0]);
					if(version_flag)
						versionbuff[version_ptr] = msg[0] & 0x0F;

					if (msg[0] == 0x0A || msg[0] == 0x20 || msg[0] == SECTION_START)
					{
						addr_flag = 1;
						version_flag = false;
						state = PARSE_ADDRESS;
...
//	VMC_LOG("\n\rSend CMD : BSL_CRC_RX_32 \n\r");
	...
	/* Append the CRC */
	uint16_t checksum = crc16(&crcPkt[3],7);
	...
	else
	{
		...
		else
		{
			...
			{
				if(receive_bufr[4] == 0x3A)
				{
					uint16_t readCRC = ((receive_bufr[6] << 8) | (receive_bufr[5]));
					if(dataCRC != readCRC)
					{
    ...
    while(1)
    {
		...
		while(upgradeStatus == STATUS_IN_PROGRESS)
		{
			...
			switch(upgradeState)
			{
				...
				case BSL_SYNCED:
				{
					uint8_t syncbuf[BSL_SYNCED_REQ] = {0xFF};
				...
				case BSL_UNLOCK_PASSWORD:
				{
					...
					else
					{
						...
						else
						{
							...
							{
								if(receive_bufr[4] == 0x3B && receive_bufr[5] == 0x00)
								{
				...
				case BSL_MASS_ERASE:
				{
					...
					uint8_t massErase[BSL_MASS_ERASE_REQ] = { 0x80,0x01,0x00,0x15,0x64,0xA3 };
...
//							VMC_LOG("\n\rProgress : %d \n\r",update_progress);
				...
				case BSL_LOAD_PC_32:
				{
					...
					uint8_t load_pc[BSL_LOAD_PC_REQ] = { 0x80,0x05,0x00,0x27,0x01,0x02,00,00,0xB8,0x66 };
				...
				case BSL_REBOOT_RESET:
				{
					...
					uint8_t rebootreset[BSL_REBOOT_RESET_REQ] = { 0x80,0x01,0x00,0x25,0x37,0x95 };
				...
				case TX_BSL_VERSION:
				{
					uint8_t version[BSL_VERSION_REQ] = { 0x80,0x01,0x00,0x19,0xE8,0x62 };
